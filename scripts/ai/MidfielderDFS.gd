...
func decide() -> Dictionary:
	...
	var sign: float = 1.0 if player.is_team_a else -1.0
	var forward: Vector2 = Vector2(player.global_position.x + 120.0 * sign, ball.global_position.y)
	var target: Vector2 = forward.lerp(ball.global_position, 0.3)
