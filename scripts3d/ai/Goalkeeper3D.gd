...
func decide() -> Dictionary:
	...
	# Keeper patrol line slightly in front of goal line (less aggressive)
	var keeper_line_x: float = goal_x - 1.2 if player.is_team_a else goal_x + 1.2
	# Box constraints (narrower and shallower)
	var max_forward_from_goal: float = 4.0
	var min_back_from_goal: float = -1.0
	var penalty_half_width_z: float = 14.0
	...
	if distance_ball_to_goal < 18.0:
		...
		if chance < 0.12:
			...
			wrong.z *= -0.6
			return {"action": "move", "direction": (dir + wrong.normalized() * 0.3)}
	...
	if close_to_ball and abs(ball.global_transform.origin.z) <= penalty_half_width_z:
		...
		return {"action": "kick", "force": 17.0, "direction": Vector3(up_dir * 9.0, 0, side)}
